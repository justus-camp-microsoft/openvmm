...
in pkgs.mkShell {
  ...
  # These are NOT set as the global RUSTFLAGS env var (which would override
  # config.toml's target-specific sections and break manual cross-builds).
  # Instead, flowey reads the appropriate per-target var at build time via
  # init_cross_build and injects it as RUSTFLAGS on the cargo command.
